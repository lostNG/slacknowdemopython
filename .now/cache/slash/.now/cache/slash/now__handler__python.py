...
import slash
__now_variables = dir(slash)


if 'handler' in __now_variables or 'Handler' in __now_variables:
    base = slash.handler if ('handler' in __now_variables) else  slash.Handler
    ...
    def now_handler(event, context):
        ...
        response = Response.from_app(slash.app, environ)
    ...
    print('Missing variable `handler` or `app` in file slash.py')
    print('See the docs https://zeit.co/docs/v2/deployments/official-builders/python-now-python')
    exit(1)
